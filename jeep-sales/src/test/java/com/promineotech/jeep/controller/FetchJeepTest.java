...
class FetchJeepTest {
	...
	class TestsThatDoNotPolluteTheApplicationContext extends FetchJeepTestSupport{
		...
		void testThatJeepsAreReturnedWhenAValidModelAndTrimAreSupplied() {
			//Given: q valid model, trim, and URI
			JeepModel model = JeepModel.WRANGLER;
			String trim  = "Sport";
//			String uri = String.format("http://localhost:%d/jeeps?model=%s&trim=%s", serverPort, model, trim);		

			String uri = String.format("%s?model=%s&trim=%s", getBaseUri(), model, trim); // Don't know which one to use, I think this one is only for videos.
...
//			String uri = String.format("http://localhost:%d/jeeps?model=%s&trim=%s", serverPort, model, trim);		
			...
			//Then: a not found (404) satus code is returned
	...
	static Stream<Arguments> parametersForInvalidInput(){
		//@formatter:off
		return Stream.of(
				arguments("WRANGLER", "!@&(*!&%$(*", "Trim contains non-alphan-numeric characters"),
